//! Supply Chain Security Simulation Tests

use core::supply_chain::{SupplyChainManager, Sbom, Component, Signature, Provenance, SourceInfo, BuildConfig, Artifact, Vulnerability};
...
fn test_realistic_supply_chain_scenario() {
    ...
    println!("✓ Signatures created and stored for {} artifacts", artifacts.len());
    ...
    let provenance = manager.create_provenance("build-20230715-001", source, build_config, build_artifacts).unwrap();
    ...
    let retrieved_provenance = manager.get_provenance(&format!("prov-{}", "build-20230715-001")).unwrap();
    assert_eq!(retrieved_provenance.build_id, "build-20230715-001");
    assert_eq!(retrieved_provenance.source.commit_hash, "a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2");
...
fn test_supply_chain_under_stress() {
    ...
    for i in 0..20 {
        let source = SourceInfo {
            ...
            tag: if i % 5 == 0 { Some(format!("v1.{}.0", i)) } else { None },
        };
        ...
        let provenance = manager.create_provenance(&format!("build-{}", i), source, build_config, artifacts).unwrap();
        assert!(manager.store_provenance(provenance).is_ok());
    }
    ...
    let vuln_count = sbom_with_vulns.components.iter()
        .map(|c| c.vulnerabilities.len())
        .sum::<usize>();
    assert_eq!(vuln_count, 5); // 100 components, every 20th has a vulnerability
    
    println!("✓ SBOM vulnerability count verified ({} vulnerabilities found)", vuln_count);
...
fn test_supply_chain_error_scenarios() {
    ...
    assert!(!manager.verify_signature("sha256:wrong-hash", &signature).unwrap());
