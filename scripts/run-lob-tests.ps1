# LOB (Limit Order Book) Test Runner
# This script runs all tests for the Orderbook Matching Engine
...
function Invoke-UnitTests {
    Write-Host "Running LOB Unit Tests..." -ForegroundColor Cyan
    
    $cmd = "forge test --match-contract OrderbookTest --match-test test"
    if ($Verbose) { $cmd += " -vvv" }
...
function Invoke-FuzzTests {
    Write-Host "Running LOB Fuzz Tests..." -ForegroundColor Cyan
    
    $cmd = "forge test --match-contract OrderbookTest --match-test testFuzz"
    if ($Verbose) { $cmd += " -vvv" }
    if ($GasReport) { $cmd += " --gas-report" }
...
function Invoke-InvariantTests {
    Write-Host "Running LOB Invariant Tests..." -ForegroundColor Cyan
    
    $cmd = "forge test --match-contract OrderbookTest --match-test invariant"
    if ($Verbose) { $cmd += " -vvv" }
    if ($GasReport) { $cmd += " --gas-report" }
...
function Invoke-AllTests {
    Write-Host "Running All LOB Tests..." -ForegroundColor Cyan
    
    $cmd = "forge test --match-contract OrderbookTest"
    if ($Verbose) { $cmd += " -vvv" }
    if ($GasReport) { $cmd += " --gas-report" }
...
function Invoke-Coverage {
    Write-Host "Running LOB Test Coverage..." -ForegroundColor Cyan
    
    $cmd = "forge coverage --match-contract OrderbookTest"
    if ($Verbose) { $cmd += " -vvv" }
...
# Main execution
Write-Host "=== LOB (Limit Order Book) Test Runner ===" -ForegroundColor Green
...
try {
    ...
    Write-Host "All LOB tests passed successfully!" -ForegroundColor Green
