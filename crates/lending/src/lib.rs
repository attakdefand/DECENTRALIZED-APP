...
impl InterestRateModel {
    ...
    pub fn calculate_supply_rate(&self, utilization: f64, borrow_rate: f64, reserve_factor: f64) -> f64 {
...
impl LendingMarket {
    ...
    pub fn new(
        ...
        Self {
            token,
            total_cash: TokenAmount { value: 0, decimals: 18 },
            total_borrows: TokenAmount { value: 0, decimals: 18 },
            total_reserves: TokenAmount { value: 0, decimals: 18 },
            interest_rate_model,
            reserve_factor,
            last_updated: 0,
        }
    ...
    pub fn utilization_rate(&self) -> f64 {
        if self.total_cash.value + self.total_borrows.value == 0 {
            ...
            self.total_borrows.value as f64 / (self.total_cash.value + self.total_borrows.value) as f64
        }
    }
    
    /// Get the current borrow interest rate (per second)
    pub fn borrow_rate_per_second(&self) -> f64 {
        let annual_rate = self.interest_rate_model.calculate_borrow_rate(self.utilization_rate());
    ...
    pub fn accrue_interest(&mut self, current_timestamp: u64) -> Result<()> {
        ...
        let interest_accumulated = (self.total_borrows.value as f64) * 
                                  (borrow_rate_per_second * time_elapsed as f64);
    ...
    pub fn borrow(&mut self, amount: &TokenAmount) -> Result<()> {
        if self.total_cash.value < amount.value {
            return Err(Error::Custom("Insufficient cash available for borrowing".to_string()));
        }
        self.total_cash.value -= amount.value;
...
impl UserPosition {
    ...
    pub fn new(user: Address, market: Address) -> Self {
        Self {
            user,
            market,
            supplied_amount: TokenAmount { value: 0, decimals: 18 },
            borrowed_amount: TokenAmount { value: 0, decimals: 18 },
            collateral_amount: TokenAmount { value: 0, decimals: 18 },
        }
    }
    
    /// Calculate the health factor of the user's position
    /// Health factor = (collateral_value * liquidation_threshold) / borrowed_value
    pub fn health_factor(&self, collateral_value: f64, borrowed_value: f64, liquidation_threshold: f64) -> f64 {
