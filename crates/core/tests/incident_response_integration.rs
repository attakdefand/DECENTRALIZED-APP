...
fn test_complete_incident_response_workflow() {
    ...
    templates.insert(
        "initial".to_string(),
        "Security incident detected: {description}. Severity: {severity}".to_string(),
    );
    templates.insert(
        "update".to_string(),
        "Incident update: {status}. Affected systems: {systems}".to_string(),
    );
    templates.insert(
        "resolution".to_string(),
        "Incident resolved: {resolution}. Post-mortem scheduled for {date}".to_string(),
    );
...
fn test_pause_kill_switch_management() {
    ...
    let switches_data = vec![
        ...
        (
            ...
            vec!["admin".to_string(), "security".to_string()],
        ),
    ];
    ...
    assert!(manager
        .deactivate_pause_kill_switch("contract-pause", "admin-user".to_string())
        .is_ok());
...
fn test_backup_restore_operations() {
    ...
    assert!(manager
        .update_restore_job_status("restore-db", RestoreStatus::InProgress, None)
        .is_ok());
    ...
    assert!(manager
        .update_restore_job_status("restore-db", RestoreStatus::Completed, None)
        .is_ok());
...
fn test_communication_plan_management() {
    ...
    let incident_types = vec![
        ...
        (
            "availability",
            "Service Availability Incident Response Plan",
        ),
    ...
    for (incident_type, description) in incident_types {
        let plan = CommunicationPlan {
            ...
            escalation_procedures: vec![EscalationStep {
                step: 1,
                time_threshold: 30,
    ...
    assert!(manager
        .get_communication_plan("availability-plan")
        .is_some());
    ...
    assert!(security_plan.is_some());
    assert_eq!(
        security_plan.unwrap().recipients[0],
