...
fn test_complete_data_integrity_workflow() {
    ...
    assert!(manager.add_content_item(content_item).is_ok(), "Content item should be added successfully");
    ...
    assert!(manager.anchor_hash(hash_anchor).is_ok(), "Hash should be anchored successfully");
    println!("✓ Hash anchored on-chain");
    
    // 5. Check pin coverage
    let coverage = manager.check_pin_coverage("QmTestContent1234567890").expect("Pin coverage check should succeed");
    assert_eq!(coverage, 100.0, "Coverage should be 100% since content is pinned on both services");
    println!("✓ Pin coverage verified: {}%", coverage);
    
    // 6. Verify content integrity
    let is_valid = manager.verify_content_integrity("QmTestContent1234567890", "QmTestContent1234567890")
    ...
    let item = manager.get_content_item("QmTestContent1234567890").expect("Content item should exist");
    assert!(manager.is_content_safe(item), "Content should be safe according to policy");
    println!("✓ Content safety verified");
    
    // 8. Retrieve stored data
    assert!(manager.get_pinning_service("pinata").is_some(), "Pinning service should be retrievable");
    assert!(manager.get_hash_anchor("QmTestContent1234567890").is_some(), "Hash anchor should be retrievable");
...
fn test_content_safety_policy_enforcement() {
    ...
    assert!(manager.add_content_item(allowed_item).is_ok(), "Allowed content should be added");
    ...
    assert!(manager.add_content_item(disallowed_item).is_err(), "Disallowed content should be rejected");
    ...
    assert!(manager.add_content_item(oversized_item).is_err(), "Oversized content should be rejected");
...
fn test_pin_coverage_calculation() {
    ...
    assert_eq!(coverage, 100.0, "Fully pinned content should have 100% coverage");
    ...
    assert_eq!(coverage, 50.0, "Partially pinned content should have 50% coverage");
...
fn test_hash_anchoring_and_retrieval() {
    ...
    for (cid, chain, tx_hash, block_number) in &anchors {
        ...
        assert!(manager.anchor_hash(anchor).is_ok(), "Hash anchoring should succeed");
    }
    ...
    for (cid, chain, tx_hash, block_number) in &anchors {
        let anchor = manager.get_hash_anchor(cid).expect("Anchored hash should be retrievable");
    ...
    assert!(manager.get_hash_anchor("QmNonExistent").is_none(), "Non-existent anchor should return None");
