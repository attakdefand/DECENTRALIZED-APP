...
use core::container_hardening::{
    ...
    SeccompAction, SeccompProfile, SecretBackend, SecretsManagement,
};
...
fn test_complete_container_hardening_workflow() {
    ...
    let apparmor_profiles = vec![AppArmorProfile {
        ...
        .into_iter()
        .collect(),
        capability_restrictions: vec!["CAP_NET_RAW".to_string(), "CAP_SYS_ADMIN".to_string()],
    }];
    ...
    assert!(manager
        .get_policy("restrict-privileged-containers")
        .is_some());
    ...
    let violations = manager
        .validate_container_config(&privileged_container)
        .unwrap();
    ...
    assert!(manager
        .remove_policy("restrict-privileged-containers")
        .is_ok());
...
fn test_container_hardening_policy_configurations() {
    ...
    assert_eq!(manager.policies.len(), 2);
    assert_eq!(
        manager.get_policy("warn-policy").unwrap().rules[0].mode,
        EnforcementMode::Warn
    );
    assert_eq!(
        manager.get_policy("audit-policy").unwrap().rules[0].mode,
    ...
    assert_eq!(
        aws_manager.secrets_management.backend,
        SecretBackend::AwsSecretsManager
    );
    assert_eq!(
        azure_manager.secrets_management.backend,
        SecretBackend::AzureKeyVault
    );
    assert_eq!(
        google_manager.secrets_management.backend,
        SecretBackend::GoogleSecretManager
    );
...
fn test_container_hardening_error_handling() {
    ...
    assert!(manager
        .get_seccomp_profile("non-existent-profile")
        .is_none());

    // Test getting non-existent AppArmor profile
    assert!(manager
        .get_apparmor_profile("non-existent-profile")
        .is_none());
