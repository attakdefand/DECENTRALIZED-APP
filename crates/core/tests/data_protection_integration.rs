...
fn test_complete_data_protection_workflow() {
    ...
    let encrypted_data = encryption.encrypt(sensitive_data).expect("Encryption should succeed");
    assert_ne!(sensitive_data, encrypted_data.as_slice(), "Data should be encrypted");
    ...
    assert!(dsr_manager.submit_request(dsr_request).is_ok(), "DSR request submission should succeed");
    println!("✓ DSR erasure request submitted");
    
    // 6. Process erasure
    assert!(dsr_manager.process_erasure("workflow-req-001").is_ok(), "DSR erasure processing should succeed");
    println!("✓ DSR erasure processed");
    
    // 7. Verify completion
    let status = dsr_manager.get_request_status("workflow-req-001").expect("Request status should exist");
    assert_eq!(status, &DsrRequestStatus::Completed, "Request should be completed");
    ...
    assert!(audit_log.iter().any(|log| log.contains("submitted")), "Audit log should contain submission entry");
    assert!(audit_log.iter().any(|log| log.contains("completed")), "Audit log should contain completion entry");
    println!("✓ Audit logging verified");
    
    // 9. Decrypt data to verify it's still intact (in real scenario, this would be erased)
    let decrypted_data = encryption.decrypt(&encrypted_data).expect("Decryption should succeed");
    assert_eq!(sensitive_data, decrypted_data.as_slice(), "Decrypted data should match original");
...
fn test_encryption_with_different_keys() {
    ...
    let encrypted1 = encryption1.encrypt(plaintext).expect("Encryption 1 should succeed");
    let encrypted2 = encryption2.encrypt(plaintext).expect("Encryption 2 should succeed");
    
    // Encrypted data should be different with different keys
    assert_ne!(encrypted1, encrypted2, "Encrypted data should differ with different keys");
    
    // Both should decrypt correctly to the same plaintext
    let decrypted1 = encryption1.decrypt(&encrypted1).expect("Decryption 1 should succeed");
    let decrypted2 = encryption2.decrypt(&encrypted2).expect("Decryption 2 should succeed");
    
    assert_eq!(decrypted1, plaintext, "Decrypted data 1 should match original");
    assert_eq!(decrypted2, plaintext, "Decrypted data 2 should match original");
...
fn test_dsr_request_filtering() {
    ...
    dsr_manager.submit_request(request1).expect("Request 1 submission should succeed");
    dsr_manager.submit_request(request2).expect("Request 2 submission should succeed");
