...
fn test_complete_supply_chain_workflow() {
    ...
    assert!(manager.store_signature(artifact_uri, signature.clone()).is_ok());
    ...
    assert_eq!(retrieved_provenance.source.repo_url, "https://github.com/example/test-app");
    ...
    let is_pinned = manager.validate_dependency_pinning(sbom_for_scanning).unwrap();
    assert!(is_pinned);
