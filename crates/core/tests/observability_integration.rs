...
fn test_complete_observability_workflow() {
    ...
    annotations.insert(
        ...
        "API latency has been above 1 second for more than 5 minutes".to_string(),
    );
    ...
    annotations.insert(
        "summary".to_string(),
        "Indexer synchronization lag detected".to_string(),
    );
    annotations.insert(
        "description".to_string(),
        "Indexer is lagging behind the chain tip by more than 10 blocks".to_string(),
    );
