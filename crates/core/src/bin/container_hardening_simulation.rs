...
use core::container_hardening::{ContainerHardeningManager, AdmissionPolicy, PolicyRule, EnforcementMode, SeccompProfile, SeccompAction, AppArmorProfile, FileAccessRule, FilePermission, NetworkAccessRule, NetworkPermission, SecretsManagement, SecretBackend, ContainerConfig, ViolationSeverity};
...
fn test_container_hardening_error_scenarios() {
    ...
    let _violations = manager_with_policies.validate_container_config(&container_with_capabilities).unwrap();
