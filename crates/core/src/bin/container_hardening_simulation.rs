//! Container Hardening Simulation Tests Binary
//!
//! This binary runs the container hardening simulation tests to verify the effectiveness
//! of our container hardening measures.

use core::container_hardening::{
    AdmissionPolicy, AppArmorProfile, ContainerConfig, ContainerHardeningManager, EnforcementMode,
    FileAccessRule, FilePermission, NetworkAccessRule, NetworkPermission, PolicyRule,
    SeccompAction, SeccompProfile, SecretBackend, SecretsManagement, ViolationSeverity,
};
...
fn test_realistic_container_hardening_scenario() {
    ...
    let seccomp_profiles = vec![SeccompProfile {
    ...
    let apparmor_profiles = vec![AppArmorProfile {
    ...
    let violations = manager
        .validate_container_config(&compliant_container)
        .unwrap();
    ...
    let non_compliant_container = ContainerConfig {
        ...
        sysctls: vec![("net.ipv4.ip_forward".to_string(), "1".to_string())]
            .into_iter()
            .collect(),
    };

    let violations = manager
        .validate_container_config(&non_compliant_container)
        .unwrap();
    ...
    assert_eq!(
        manager.secrets_management.backend,
        SecretBackend::HashiCorpVault
    );
...
fn test_container_hardening_under_stress() {
    ...
    for i in 0..20 {
        let policy = AdmissionPolicy {
            ...
            rules: vec![PolicyRule {
                name: if i == 0 {
                    // Make the first rule check for privileged containers
                    "no-privileged-rule".to_string()
                } else {
                    format!("rule-{}", i)
                },
                ...
                expression: if i == 0 {
                    // Make the first rule check for privileged containers
                    "container.securityContext.privileged == false".to_string()
                } else {
                    format!("container.label.{} == true", i)
                },
                mode: if i % 3 == 0 {
    ...
    for i in 0..10 {
        let profile = SeccompProfile {
            ...
            allowed_syscalls: vec!["read".to_string(), "write".to_string(), "close".to_string()],
...
fn test_container_hardening_error_scenarios() {
    ...
    assert_eq!(
        manager.secrets_management.backend,
        SecretBackend::KubernetesSecrets
    );
    ...
    let mut manager_with_policies = ContainerHardeningManager::new(SecretsManagement {
        backend: SecretBackend::KubernetesSecrets,
        rotation_interval: 86400,
        encryption_enabled: true,
        access_logging_enabled: true,
    ...
    manager_with_policies.add_policy(policy);
    ...
    let _violations = manager_with_policies
        .validate_container_config(&container_with_capabilities)
        .unwrap();
