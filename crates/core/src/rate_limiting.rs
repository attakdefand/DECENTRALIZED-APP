...
impl RateLimiter {
    ...
    pub fn is_allowed(&mut self, identifier: &str) -> bool {
        let bucket = self
            ...
            .or_insert_with(|| {
                TokenBucket::new(
                    self.default_bucket.max_tokens,
                    self.default_bucket.refill_rate,
                )
            });
    ...
    {
        {
            ...
            self.cache
                .retain(|_, record| record.timestamp.elapsed() < self.ttl);
...
mod tests {
    ...
    fn test_idempotency_manager() {
        ...
        let result: Result<String, String> =
            manager.process_request("key1", || Ok("Hello, World!".to_string()));
    ...
    async fn test_job_guard() {
        ...
        let result = guard
            .execute_job::<&str, _, anyhow::Error>(async { Ok("Job completed") })
            .await;
