...
fn test_complete_bridge_security_workflow() {
    ...
    assert!(light_client_bridge
        .add_challenger(challenger.clone())
        .is_ok());
    ...
    assert!(light_client_bridge
        .relay_transfer("test-transfer-001", vec![1, 2, 3])
        .is_ok());
    ...
    assert!(light_client_bridge
        .submit_challenge(challenge.clone())
        .is_ok());
    assert!(optimistic_bridge.challenge_transfer(challenge).is_ok());
    assert!(light_client_bridge
        .resolve_challenge("test-challenge-001", true)
        .is_ok());
    assert!(optimistic_bridge
        .resolve_challenge("test-challenge-001", false)
        .is_ok());
...
fn test_bridge_security_under_stress() {
    ...
    for i in 0..5 {
        assert!(bridge
            .resolve_challenge(&format!("stress-challenge-{}", i), i % 2 == 0)
            .is_ok());
    }
...
fn test_bridge_security_edge_cases() {
    ...
    assert!(opt_bridge
        .resolve_challenge("min-stake-challenge", true)
        .is_ok());
    ...
    assert!(opt_bridge
        .resolve_challenge("edge-challenge", false)
        .is_ok());
    assert!(opt_bridge
        .resolve_challenge("multi-challenge-1", true)
        .is_ok());
    assert!(opt_bridge
        .resolve_challenge("multi-challenge-2", false)
        .is_ok());
