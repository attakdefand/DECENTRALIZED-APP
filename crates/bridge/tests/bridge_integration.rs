...
fn test_complete_zk_bridge_workflow() {
    ...
    let transfer_id = bridge.submit_transfer_with_proof(transfer, vec![1, 2, 3, 4, 5]).unwrap();
...
fn test_bridge_security_error_handling() {
    ...
    assert!(bridge.submit_challenge(challenge_with_nonexistent_challenger).is_err());
